...
export default {
   ...
   run(bot: CommandBot, authProfile: chatterProfile) {
      bot.emit('getChatterProfiles', (chatterProfiles: chatterProfile[]) => {
         const bannable = chatterProfiles.filter(
            (chatter: chatterProfile) =>
               chatter.roles.length === 0 &&
               chatter.peerId !== bot.peerId &&
               chatter.peerId !== authProfile.peerId
         )
         if (bannable.length > 0) {
            ...
            bot.emit(
               'chat',
               `HAHAHAHAHAH j'ai ban toute la game t'es le dieu mtn`
            )
         } else {
