...
export default {
    ...
    run(bot: CommandBot) {
        bot.emit("getChatterProfiles", (chatterProfiles: chatterProfile[]) => {
            const chatters = chatterProfiles.filter((chatter: chatterProfile) => chatter.roles.length == 0);
            if (chatters.length > 0) {
                const chatter = chatters[Math.floor(Math.random() * chatters.length)];
                bot.emit("chat", `On m'annonce que ${chatter.nickname} est une grosse pute`);
            } else {
