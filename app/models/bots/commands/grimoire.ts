        ...
        switch (args[0]) {
            case "set":
                ...
                if (!grimoire.debitage) {
                    ...
                    if (grimoire.debitage.find((e: string) => e === debitage)) return bot.emit("chat", `Ce texte est déjà enregistré dans le grimoire enchanté`);
                    grimoire.debitage.push(debitage);
                }
                bot.emit("chat", `Et un texte de plus que j'enregistre pour que tu puisses continuer de régner sur ce jeu de fdp`);
            ...
            case "get":
                ...
                if (!grimoireGet.debitage) return bot.emit("chat", `Y a aucun poème dans le grimoire pour le moment`);
                const found = grimoireGet.debitage.find((e: string) => e.toLowerCase().includes(search.toLocaleLowerCase())).replace(/{name}/g, name);
                bot.emit("chat", found);
                break;
        };
    }
