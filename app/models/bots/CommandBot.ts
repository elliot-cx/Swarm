   ...
   handleCommand(authProfile: chatterProfile, message: string): void {
      this.emit('getChatterProfiles', (chatterProfiles: chatterProfile[]) => {
         if (message.startsWith('/') && authProfile.peerId !== this.peerId) {
            if (
               authProfile.roles.length === 0 &&
               authProfile.nickname !== 'Jacques Ulé' &&
               authProfile.nickname !== 'Guest3108'
            )
               return this.emit('chat', "T'es pas mod sale fdp")
            const args = message.split(' ')
            const command = args.shift()?.replace('/', '')
            const commands = readdirSync('./app/models/bots/commands/')
            if (!commands.includes((command as string) + '.ts'))
               return this.emit('chat', `Je connais pas cette commande frérot`)
            const file = require(`./commands/${command}`).default
            if (
               file.needMod &&
               (
                  chatterProfiles.find(
                     (c) => c.peerId === this.peerId
                  ) as chatterProfile
               ).roles.length < 1
            )
               return this.emit('chat', 'Faut que je sois mod imbécile va')
